...
namespace Mre.OTI.Presupuesto.Api.Controllers  // institucionales  Institucionales  AccionesInstitucionales
{
    [Route("v1/rree/spp/presupuesto/seg/accionesinstitucionales")]
    [ApiController]
    [Authorize]
    [AuthorizeCheckActionFilter]
    public class AccionesInstitucionalesController : ControllerBase
    {
        private readonly IMediator _IMediator;
        public AccionesInstitucionalesController(IMediator IMediator)
        {
        ...
        public async Task<IActionResult> ObtenerAccionesInstitucionalesPaginado([FromQuery] ObtenerAccionesInstitucionalesPaginadoViewModel request)
        {
        ...
        [HttpGet]
        [Route("obtener")]
        public async Task<IActionResult> ObtenerAccionesInstitucionales([FromQuery] ObtenerAccionesInstitucionalesViewModel request)
        {
            request.usuarioConsulta = Request.Headers.ContainsKey(Constantes.SISTEMA.VAR_AUDITORIA_HEADER) ? Request.Headers.GetOrDefault(Constantes.SISTEMA.VAR_AUDITORIA_HEADER).ToString() : Constantes.SISTEMA.VAR_VAL_ZERO_ENCRYPT;
            var result = await _IMediator.Send(request);
            return Ok(result);
        ...
        [HttpPost]
        [Route("guardar")]
        public async Task<IActionResult> agregar([FromBody] AgregarAccionesInstitucionalesViewModel request)
        {
        ...
        [HttpPut]
        [Route("actualizar")]
        public async Task<IActionResult> actualizar([FromBody] ActualizarAccionesInstitucionalesViewModel request)
        {
        ...
        [HttpPut]
        [Route("eliminar")]
        public async Task<IActionResult> eliminar([FromBody] EliminarAccionesInstitucionalesViewModel request)
        {
            request.usuarioModificacion = Request.Headers.ContainsKey(Constantes.SISTEMA.VAR_AUDITORIA_HEADER) ? Request.Headers.GetOrDefault(Constantes.SISTEMA.VAR_AUDITORIA_HEADER).ToString() : Constantes.SISTEMA.VAR_VAL_ZERO_ENCRYPT;
            var result = await _IMediator.Send(request);
            return Ok(result);
        ...
        [HttpGet]
        [Route("listado")]
        public async Task<IActionResult> ObtenerListadoAcciones([FromQuery] ObtenerListadoAccionesInstitucionalesViewModel request) 
        {
