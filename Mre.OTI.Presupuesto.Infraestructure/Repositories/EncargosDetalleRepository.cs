...
{
    public class EncargosDetalleRepository : IEncargosDetalleRepository
    {
        readonly DBConnection DBConnection;

        public EncargosDetalleRepository(DBConnection _DBConnection)
        {
            this.DBConnection = _DBConnection;
        }

        public async Task<int> Guardar(EncargosDetalle parametro)
        {
            const string sql = @"SC_SPP.MAESI_PROGRAMACION_ENCARGOS_DETALLE";
            int result = 0;

            var parameters = new DynamicParameters();
            parameters.Add("@ID_PROGRAMACION_ENCARGOS_DETALLE", parametro.ID_PROGRAMACION_ENCARGOS_DETALLE, DbType.Int32);
