...
namespace Mre.OTI.Presupuesto.Infraestructure.Repositories
{
    public class ProyectoRepository : IProyectoRepository
    {
        ...
        public ProyectoRepository(DBConnection _DBConnection)
        {
            this.DBConnection = _DBConnection;
        }

        public async Task<IEnumerable<ObtenerListadoProyectoResponseDTO>> ObtenerListadoProyecto(ObtenerListadoProyectoRequestDTO request)
        {
            const string sql = @"SC_SPP.MAESS_LISTAR_PROGRAMACION_PROYECTO";
            ...
            parameters.Add("@ID_ACTIVIDAD_OPERATIVA", request.idActividadOperativa, DbType.Int32);

            var result = await DBConnection.Connection.QueryAsync<ObtenerListadoProyectoResponseDTO>(
                sql,
                parameters,
        ...
        public async Task<IEnumerable<ObtenerProyectoPaginadoResponseDTO>> ObtenerProyectoPaginado(ObtenerProyectoPaginadoRequestDTO request)
        {
            const string sql = @"SC_SPP.MAESS_PAGINADO_PROGRAMACION_PROYECTO";
            ...
            var result = await DBConnection.Connection.QueryAsync<ObtenerProyectoPaginadoResponseDTO>(
                sql,
                parameters,
                DBConnection.Transaction,
                commandType: CommandType.StoredProcedure
        ...
        public async Task<ObtenerProyectoPorIdResponseDTO> ObtenerProyectoPorId(int idProgramacionProyecto, string usuarioConsulta)
        {
            const string sql = @"SC_SPP.MAESS_OBTENERID_PROGRAMACION_PROYECTO";

            var parameters = new DynamicParameters();
            parameters.Add("@ID_PROGRAMACION_PROYECTO", idProgramacionProyecto, DbType.Int32);

            var result = await DBConnection.Connection.QueryFirstOrDefaultAsync<ObtenerProyectoPorIdResponseDTO>(
                sql,
                parameters,
                DBConnection.Transaction,
                commandType: CommandType.StoredProcedure
            ...
            if (result == null)
            {
                throw new KeyNotFoundException($"No se encontró proyecto con ID {idProgramacionProyecto}");
                // O retornar un objeto vacío: return new ObtenerProyectoPorIdResponseDTO();
            }

            return result;
        }

        public async Task<int> Guardar(Proyecto parametro)
        {
            const string sql = @"SC_SPP.MAESI_PROGRAMACION_PROYECTO";
            int result = 0;
        ...
        public async Task<int> Actualizar(Proyecto parametro)
        {
            const string sql = @"SC_SPP.MAESU_PROGRAMACION_PROYECTO";
            int result = 0;

            var parameters = new DynamicParameters();
            parameters.Add("@ID_PROGRAMACION_PROYECTO", parametro.ID_PROGRAMACION_PROYECTO, DbType.Int32);
            parameters.Add("@ID_PROGRAMACION_TAREAS", parametro.ID_PROGRAMACION_TAREAS, DbType.Int32);
            parameters.Add("@ID_ANIO", parametro.ID_ANIO, DbType.Int32);
            parameters.Add("@ID_ACTIVIDAD_OPERATIVA", parametro.ID_ACTIVIDAD_OPERATIVA, DbType.Int32);
            parameters.Add("@ID_TAREA", parametro.ID_TAREA, DbType.Int32);
        ...
        public async Task<int> Eliminar(Proyecto parametro)
        {
            const string sql = @"SC_SPP.MAESD_PROGRAMACION_PROYECTO";
            int result = 0;
