...
{
    public class ProyectoDetalleRepository : IProyectoDetalleRepository
    {
        readonly DBConnection DBConnection;

        public ProyectoDetalleRepository(DBConnection _DBConnection)
        {
            this.DBConnection = _DBConnection;
        }

        public async Task<int> Guardar(ProyectoDetalle parametro)
        {
            const string sql = @"SC_SPP.MAESI_PROGRAMACION_PROYECTO_DETALLE";
            int result = 0;

            var parameters = new DynamicParameters();
            parameters.Add("@ID_PROGRAMACION_PROYECTO_DETALLE", parametro.ID_PROGRAMACION_PROYECTO_DETALLE, DbType.Int32);
