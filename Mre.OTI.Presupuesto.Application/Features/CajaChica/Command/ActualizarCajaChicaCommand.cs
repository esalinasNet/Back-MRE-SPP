...
{
    public class ActualizarCajaChicaCommand : IRequestHandler<ActualizarCajaChicaViewModel, CommandResponseViewModel>
    {
        private readonly ICajaChicaRepository _ICajaChicaRepository;
        private readonly IUsuarioRolRepository _IUsuarioRolRepository;

        public ActualizarCajaChicaCommand(
        ...
        public async Task<CommandResponseViewModel> Handle(ActualizarCajaChicaViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _ICajaChicaRepository.Actualizar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? "Caja Chica actualizada correctamente" : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
