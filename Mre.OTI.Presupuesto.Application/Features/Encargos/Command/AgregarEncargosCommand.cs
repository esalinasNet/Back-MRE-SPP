...
{
    public class AgregarEncargosCommand : IRequestHandler<AgregarEncargosViewModel, CommandResponseViewModel>
    {
        private readonly IEncargosRepository _IEncargosRepository;
        private readonly IUsuarioRolRepository _IUsuarioRolRepository;

        public AgregarEncargosCommand(
        ...
        public async Task<CommandResponseViewModel> Handle(AgregarEncargosViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IEncargosRepository.Guardar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? "Encargos registrado correctamente" : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
