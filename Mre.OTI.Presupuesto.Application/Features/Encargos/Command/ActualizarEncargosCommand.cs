...
{
    public class ActualizarEncargosCommand : IRequestHandler<ActualizarEncargosViewModel, CommandResponseViewModel>
    {
        private readonly IEncargosRepository _IEncargosRepository;
        private readonly IUsuarioRolRepository _IUsuarioRolRepository;

        public ActualizarEncargosCommand(
        ...
        public async Task<CommandResponseViewModel> Handle(ActualizarEncargosViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IEncargosRepository.Actualizar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? "Encargos actualizado correctamente" : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
