...
{
    public class EliminarViaticosCommand : IRequestHandler<EliminarViaticosViewModel, CommandResponseViewModel>
    {
        private readonly IViaticosRepository _IViaticosRepository;
        private readonly IUsuarioRolRepository _IUsuarioRolRepository;

        public EliminarViaticosCommand(
        ...
        public async Task<CommandResponseViewModel> Handle(EliminarViaticosViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IViaticosRepository.Eliminar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? "Viático eliminado correctamente" : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
