...
{
    public class ActualizarPaisesCommand : IRequestHandler<ActualizarPaisesViewModel, CommandResponseViewModel>
    {
        readonly private IPaisesRepository _IPaisesRepository;
        readonly private IUsuarioRolRepository _IUsuarioRolRepository;

        public ActualizarPaisesCommand(IPaisesRepository IPaisesRepository, IUsuarioRolRepository IUsuarioRolRepository)
        {
            _IPaisesRepository = IPaisesRepository;
            _IUsuarioRolRepository = IUsuarioRolRepository;
        }
        public async Task<CommandResponseViewModel> Handle(ActualizarPaisesViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IPaisesRepository.Actualizar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? Constantes.MensajesOK.M01_PAIS_UPDATE_OK : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
