...
{
    public class ActualizarProgramaCommand : IRequestHandler<ActualizarProgramaViewModel, CommandResponseViewModel>
    {
        readonly private IProgramaRepository _IProgramaRepository;
        readonly private IUsuarioRolRepository _IUsuarioRolRepository;

        public ActualizarProgramaCommand(IProgramaRepository IProgramaRepository, IUsuarioRolRepository IUsuarioRolRepository)
        {
            _IProgramaRepository = IProgramaRepository;
            _IUsuarioRolRepository = IUsuarioRolRepository;
        }
        public async Task<CommandResponseViewModel> Handle(ActualizarProgramaViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IProgramaRepository.Actualizar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? Constantes.MensajesOK.M01_PROGRAMA_UPDATE_OK : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
