...
{
    public class ActualizarCalendarioCommand : IRequestHandler<ActualizarCalendarioViewModel, CommandResponseViewModel>
    {
        readonly private ICalendarioRepository _ICalendarioRepository;
        private readonly IMapper _mapper;
        readonly private IUsuarioRolRepository _IUsuarioRolRepository;
        public ActualizarCalendarioCommand(IUsuarioRolRepository IIUsuarioRolRepository, ICalendarioRepository ICalendarioRepository, IMapper mapper)
        {
        ...
        public async Task<CommandResponseViewModel> Handle(ActualizarCalendarioViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                //if (string.IsNullOrEmpty(request.descripcion)) throw new MreException(Constantes.MensajesError.EX_CALENDARIO_DESCRIPCION_REQUIRED);
                ...
                var result = await _ICalendarioRepository.Actualizar(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? Constantes.MensajesOK.M01_CALENDARIO_UPDATE_OK : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
