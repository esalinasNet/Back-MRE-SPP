...
{
    public class ActualizarUbigeoExteriorCommand : IRequestHandler<ActualizarUbigeoExteriorViewModel, CommandResponseViewModel>
    {
        readonly private IUbigeoExteriorRepository _IUbigeoExteriorRepository;
        private readonly IMapper _mapper;
        readonly private IUsuarioRolRepository _IUsuarioRolRepository;
        public ActualizarUbigeoExteriorCommand(IUsuarioRolRepository IIUsuarioRolRepository, IUbigeoExteriorRepository IUbigeoExteriorRepository, IMapper mapper)
        {
        ...
        public async Task<CommandResponseViewModel> Handle(ActualizarUbigeoExteriorViewModel request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _IUbigeoExteriorRepository.ActualizarUbigeoExterior(entity);

                return new CommandResponseViewModel
                {
                    message = result > 0 ? Constantes.MensajesOK.M01_UBIGEO_EXTERIOR_UPDATE_OK : Constantes.MensajesError.EX_ERROR_GENERICO,
                    result = result
                };
            }
            catch (Exception ex)
